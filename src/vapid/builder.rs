...
/// # }
/// ```
pub struct VapidSignatureBuilder<'a, 'keys> {
    claims: Claims,
    key: VapidKey,
    subscription_info: &'a SubscriptionInfo<'keys>,
}

impl<'a, 'keys> VapidSignatureBuilder<'a, 'keys> {
    ...
    pub fn from_pem<R: Read>(
        pk_pem: R,
        subscription_info: &'a SubscriptionInfo<'keys>,
    ) -> Result<VapidSignatureBuilder<'a, 'keys>, WebPushError> {
    ...
    /// Creates a new builder from a raw private key, that is the literal bytes of the private key itself.
    pub fn from_bytes(
        bytes: &[u8],
        subscription_info: &'a SubscriptionInfo<'keys>,
    ) -> Result<VapidSignatureBuilder<'a, 'keys>, WebPushError> {
        let pr_key = ES256KeyPair::from_bytes(bytes).map_err(|_| WebPushError::InvalidCryptoKeys)?;
